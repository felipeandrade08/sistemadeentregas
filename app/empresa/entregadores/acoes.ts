...
export async function createDriver(name: string, phone: string) {
  ...
  const { error } = await ctx.supabase.from('drivers').insert({ company_id: ctx.companyId, name: name.trim(), phone: phone.trim() || null, is_active: true })
...
export async function toggleDriver(driverId: string, isActive: boolean) {
  const ctx = await companyId()
  if (ctx.error) return { error: ctx.error }
  const { error } = await ctx.supabase.from('drivers').update({ is_active: isActive }).eq('id', driverId).eq('company_id', ctx.companyId)
  if (error) return { error: error.message }
  revalidatePath('/empresa/entregadores')
...
export async function assignDriver(orderId: string, driverId: string) {
  const ctx = await companyId()
  if (ctx.error) return { error: ctx.error }
  const { data: driver } = await ctx.supabase.from('drivers').select('id,name').eq('id', driverId).eq('company_id', ctx.companyId).eq('is_active', true).maybeSingle()
