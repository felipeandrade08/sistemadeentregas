...
export default function ConfigurarEmpresaPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.rpc('create_company_for_current_user', {
      p_name: name,
      p_slug: makeSlug(slug || name),
      p_phone: phone || null,
      p_address: address || null,
    })

    if (error) {
      setError(error.code === '23505' ? 'Este endereço da loja já está em uso. Escolha outro.' : error.message)
      setLoading(false)
      return
    }
  ...
  return (
    <main className="auth-page">
      <section className="auth-card" style={{ maxWidth: 560 }}>
        ...
        <p className="muted">Esses dados serão usados para criar sua loja e organizar seus pedidos.</p>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <label>Endereço da loja<input value={slug} onChange={(e) => setSlug(makeSlug(e.target.value))} placeholder="mercado-do-felipe" /></label>
          <small className="muted">Sua loja ficará disponível futuramente em /cliente?empresa={makeSlug(slug || name) || 'sua-empresa'}</small>
