...
export default async function EmpresaPage() {
  ...
  const { data: orders } = await supabase.from('orders').select('id, order_number, status, total, created_at, payment_method, delivery_fee, driver_id, drivers(name)').eq('company_id', profile.company_id).order('created_at', { ascending: false }).limit(100)
  ...
  return <main className="dashboard">
    ...
    <header className="dashboard-header"><div className="dashboard-brand"><span>E</span> ENTREGAOS</div><div className="dashboard-nav"><Link href="/empresa">Pedidos</Link><Link href="/empresa/produtos">Produtos</Link><Link href="/empresa/entregadores">Entregadores</Link><div className="status-pill">● Sistema online</div></div></header>
